...
def load_from_folder(folder: Path, include_depth: bool, limit: int = 1000, recurrency_size: int = 1):
...
def load_test_experiment(architecture: str, include_depth: bool, include_state: bool = True,  train_mode: str = "random"):
    ...
    if architecture.lower() == "vit":
        ...
        # Choosing the model
        model = ForceEstimatorVS(rs_size=26, num_layers=50, pretrained=False, att_type='BAM', include_depth=include_depth) if include_state else ForceEstimatorV(num_layers=50, pretrained=False, att_type='BAM', include_depth=include_depth)

    elif architecture.lower() == 'cnn':
        print("LOADING EXPERIMENT [=>   ]")
        print("Chosen model is ResNet50 (CNN) {}".format("vision+state" if include_state else "vision only"))
        # Choosing the model
        model = ForceEstimatorVS(rs_size=26, num_layers=50, pretrained=False, include_depth=include_depth) if include_state else ForceEstimatorV(num_layers=50, pretrained=False, include_depth=include_depth)

    elif architecture.lower() == 'rnn':
        print("LOADING EXPERIMENT [=>   ]")
        print("Chosen model is ResNet50 (RNN) {}".format("vision+state" if include_state else "vision only"))
        # Choosing the model
        model = RecurrentCNN(num_layers=50, pretrained=False, include_depth=include_depth)

    elif architecture.lower() == 'rnn-bam':
        print("LOADING EXPERIMENT [=>   ]")
        print("Chosen model is ResNet50 (RNN) {}".format("vision+state" if include_state else "vision only"))
        # Choosing the model
        model = RecurrentCNN(num_layers=50, pretrained=False, include_depth=include_depth, att_type='BAM')
    ...
    if architecture.lower() == 'fc':
        ...
        checkpoints = checkpoints_root/"{}/{}/{}_{}".format("rgb", architecture, "visu_state" if include_state else "visu", train_mode)
    else:
