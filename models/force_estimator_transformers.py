...
def sample_gumbel(shape, device: device, dtype, eps = 1e-6):
...
class AdaptiveTokenSampling(nn.Module):
    def __init__(self, output_num_tokens, eps: float = 1e-6) -> None:
        super().__init__()
        self.eps = eps
        self.output_num_tokens = output_num_tokens

    def forward(self, attn: Tensor, value, mask):
...
class Transformer(nn.Module):
    def __init__(self, 
                dim: int, 
                depth: int, 
                max_tokens_per_depth: Tuple[int], 
                heads: int, 
                dim_head: int, 
                mlp_dim: int, 
                dropout: float = 0.) -> None:
