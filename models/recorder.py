...
class BamRecorderV(nn.Module):
    ...
    def _hook(self, _, input, output):
        self.recording.append(output.clone().detach())
    
    def _register_hook(self):
        attn_modules = find_modules(self.model.encoder, BAM)

        for attn_module in attn_modules:
            handle = attn_module.spatial_att.register_forward_hook(self._hook)
            self.hooks.append(handle)
        self.hook_registered = True
    ...
    def forward(self, img: torch.Tensor):
        assert not self.ejected, 'recorder had been ejected, cannot be used anymore'
        self.clear()
        if not self.hook_registered:
            self._register_hook()
        ...
        target_device = self.device if self.device is not None else img.device
        recording = tuple(map(lambda t: t.to(target_device), self.recording))

        attns = [rec for rec in recording]

        return pred, attns


class BamRecorderVS(nn.Module):
    ...
    def forward(self, img: torch.Tensor, robot_state: torch.Tensor):
        assert not self.ejected, 'recorder had been ejected, cannot be used anymore'
        self.clear()
        if not self.hook_registered:
            self._register_hook()
        
        pred = self.model(img, robot_state)
