...
class RandomScaleCrop(object):
    ...
    def __call__(self, image: np.ndarray, intrinsics):
        assert intrinsics is not None
        output_intrinsics = np.copy(intrinsics)
...
class ColorJitter(object):
    ...
    def get_params(self, brightness: float, contrast: float, saturation: float, hue: float):
        if brightness > 0:
    ...
    def __call__(self, image: np.ndarray, intrinsics: np.ndarray) -> list:
        assert intrinsics is not None
